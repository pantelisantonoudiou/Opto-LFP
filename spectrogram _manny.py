...
if __name__ == '__main__':
    ...
    index = index[index['stim_hz']<150]
    # map index frequencies to correct stim values
    index['stim_hz'] = index.groupby('stim_hz', group_keys=False)['stim_hz'].apply(pd.cut, bins=[3, 7, 15, 23, 29, 33, 37, 41, 46, 51, 65, 74, 84, 94, 104, 154, 204, 504, 1004], 
                                                                                 labels=[5, 10, 20, 25, 30, 35, 40, 45, 50, 60, 70, 80, 90, 100, 150, 200, 500, 1000]).astype(int)
...
g = sns.relplot(data=index, x='stim_hz', y='power_ratio', errorbar='se', hue='animal_id',
             row='brain_region', kind='line', marker='o') #col='laser',
